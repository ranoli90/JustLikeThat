      ...
      <Card>
        <CardContent className="pt-6">
          <div className="flex items-center justify-between">
            ...
            <div className={`rounded-lg px-6 py-3 ${gradeColors[grade]}`}>
            ...
            {scores.map((score, index) => (
              <div key={index} className="space-y-2">
                ...
                {score.feedback.length > 0 && (
                  <p className="text-xs text-gray-500">{score.feedback.at(0)}</p>
                )}
              </div>
            ))}
          </div>
              ...
              <ul className="space-y-2">
                {strengths.map((strength, index) => (
                  <li key={index} className="flex items-start space-x-2">
                    <svg className="mt-0.5 size-5 text-green-500" fill="currentColor" viewBox="0 0 20 20">
            ...
            <CardContent>
              <ul className="space-y-2">
                {weaknesses.map((weakness, index) => (
                  <li key={index} className="flex items-start space-x-2">
                    <svg className="mt-0.5 size-5 text-red-500" fill="currentColor" viewBox="0 0 20 20">
            ...
            <ul className="space-y-3">
              {recommendations.map((recommendation, index) => (
                <li key={index} className="flex items-start space-x-3">
                  <span className="flex size-6 shrink-0 items-center justify-center rounded-full bg-blue-100 text-sm font-medium text-blue-600">
                    {index + 1}
                  </span>
